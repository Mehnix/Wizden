...
public sealed partial class QuickAnchorSystem : EntitySystem
{
    ...
    /// Don't display it if we're on able to anchor/unanchor
    /// </summary>
    ...
    private bool CanAnchor(Entity<QuickAnchorComponent> ent, TransformComponent xform)
    {
        ...
        if (xform.Anchored && (ent.Comp.Flags & AnchorableFlags.Unanchorable) == 0x0) //if we are anchored and don't have the unanchorable flag, can't unanchor
            return false;

        return !(Transform(ent).GridUid == null || _anchor.CanAnchorAt(ent.Owner)); //must be on a grid, must not be colliding with something
    }
    ...
    {
        if (xform.GridUid == null) //if not on a grid, stop immediately
            return;
